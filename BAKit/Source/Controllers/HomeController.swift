...
class HomeController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
  ...
  lazy var buttonClose: UIBarButtonItem = {
    ...
    let btn = UIBarButtonItem(image: img, style: UIBarButtonItem.Style.plain, target: self, action: #selector(hideBoardActive(_:)))
    btn.tintColor = .black
    return btn
  }()
  // [END]
  ...
  private func setupStyles() {
    ...
    collectionView?.contentInset = UIEdgeInsets(top: 50, left: 0, bottom: 0, right: 0)          // Top menu is 50 px tall
